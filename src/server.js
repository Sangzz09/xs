// ==============================
// 🌟 BOTRUMSUNWIN API VIP - FULL TIẾNG VIỆT + TỶ LỆ CHÍNH XÁC 🌟
// ==============================
...
// 🔮 Thuật toán VIP nâng cấp
function predictAdvanced(hist) {
  if (hist.length < 10)
    return { du_doan: Math.random() > 0.5 ? "Tài" : "Xỉu", thuat_toan: "Ngẫu nhiên (ít dữ liệu)" };
  ...
  for (let len = 2; len <= 5; len++) {
    ...
    const seq = recent.slice(-len).map(h => h.ket_qua).join("");
    const matches = [];
    for (let i = 0; i <= recent.length - len - 1; i++) {
      const sub = recent.slice(i, i + len).map(h => h.ket_qua).join("");
      if (sub === seq) matches.push(recent[i + len].ket_qua);
    }
    matches.forEach(r => rolling[r] += 1);
  }

  // --- Pattern matching ---
  const seq20 = recent.map(h => (h.ket_qua === "Tài" ? "t" : "x")).join("");
  ...
  // --- Bias cân bằng ---
  const taiCount = recent.filter(h => h.ket_qua === "Tài").length;
  let biasScore = 0;
  ...
  for (let i = recent.length - 2; i >= 0; i--) {
    if (recent[i].ket_qua === recent[i + 1].ket_qua) streak++;
    else break;
  }
  const streakScore = streak >= 3 ? -2 : 1;

  // --- Momentum ---
  const trend = recent.map(r => (r.ket_qua === "Tài" ? 1 : -1)).reduce((a, b) => a + b, 0);
  ...
  if (hist.length >= 23) {
    ...
    const wrongCount = last3.filter(h => h.du_doan && h.du_doan !== h.ket_qua).length;
    if (wrongCount >= 2)
      return { du_doan: last.ket_qua === "Tài" ? "Xỉu" : "Tài", thuat_toan: "Cân bằng lại sau chuỗi sai" };
  }
...
// 🔹 Tính thống kê đúng/sai và tỉ lệ
function calcStats() {
  let correct = 0, wrong = 0;
  fullHistory.forEach(h => {
    if (h.du_doan && h.ket_qua) {
      if (h.du_doan === h.ket_qua) correct++;
      else wrong++;
    }
  });
  const tong = correct + wrong;
  const tile = tong === 0 ? 0 : ((correct / tong) * 100).toFixed(2);
  return { tong_so_phien: fullHistory.length, so_dung: correct, so_sai: wrong, ti_le_dung: tile + "%" };
}
  ...
  try {
    ...
    if (!isNaN(phien) && tong >= 3 && tong <= 18) {
      if (!fullHistory.find(h => h.phien === phien)) {
        ...
        saveHistory();
        console.log(`✅ Phiên ${phien}: ${ket_qua} | Dự đoán: ${du_doan}`);
      }
    }
  } catch (err) {
...
app.get("/fullhistory", (req, res) => {
  res.json({
    tong_so_phien: fullHistory.length,
    fullHistory
  });
});
