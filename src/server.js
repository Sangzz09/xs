...
app.get("/sunwinapi", (req, res) => {
  try {
    ...
    return res.json({
      Phiên_hiện_tại: last.phien,
      Kết_quả_hiện_tại: last.ket_qua,
      Xúc_xắc: last.xuc_xac,
      Tổng_xúc_xắc: last.tong_xuc_xac,
      Dự_đoán_đã_thực_hiện: last.du_doan,
      Kết_quả_dự_đoán: last.du_doan === last.ket_qua ? "Đúng ✓" : "Sai ✗",
      
      Phiên_tiếp_theo: last.phien + 1,
      Dự_đoán_phiên_tiếp_theo: nextDecision.du_doan,
      Cầu_hiện_tại: nextDecision.details?.pattern?.name || "Không rõ",
      Confidence_phiên_tiếp_theo: Math.round(nextDecision.confidence * 100) + "%",
      Lý_do: nextDecision.reason,
      
      Pattern_10_phiên: seq10,
      Pattern_30_phiên: seq30,
      Thuật_toán: `HybridPlus v16 Enhanced`,
      
      Thống_kê: {
        Tỷ_lệ_thắng: `${acc}%`,
        Tổng_số_dự_đoán: data.stats.tong,
        Số_đúng: data.stats.dung,
        Số_sai: data.stats.sai,
        Chuỗi_thắng: data.flow.lastWins || 0,
        Chuỗi_thua: data.flow.lastLosses || 0
      },
      
      Phát_triển_bởi: "@minhsangdangcap"
    });
  } catch (e) {
