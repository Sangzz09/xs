...
function decideFromSeqAndLocal(seq, localHistory) {
  ...
  // ======== VOTING SYSTEM - Chỉ giữ 3 thuật toán CHÍNH ========
  let taiVotes = 0, xiuVotes = 0;
  let reasons = [];
  
  // AI HTDD (weight: 0.4)
  if (aiHtdd.prediction === 'Tài') { 
    taiVotes += 0.4; 
    reasons.push(`AI:Tài`); 
  } else { 
    xiuVotes += 0.4; 
  ...
  if (markovScore > 0) { 
    taiVotes += 0.35; 
    reasons.push(`Markov:Tài`); 
  } else { 
    xiuVotes += 0.35; 
    reasons.push(`Markov:Xỉu`); 
  }
  
  // Momentum (weight: 0.25)
  if (momentum.score > 0) { 
    taiVotes += 0.25; 
    reasons.push(`Momentum:Tài`); 
  } else { 
    xiuVotes += 0.25; 
    reasons.push(`Momentum:Xỉu`); 
  }
  ...
  const reasonStr = `Voting ${pick} (${(voteRatio*100).toFixed(0)}%) | ${aiHtdd.reason}`;
  ...
  return { 
    ...
    details: { 
      ...
      votes: { taiVotes: taiVotes.toFixed(2), xiuVotes: xiuVotes.toFixed(2), reasons }
    } 
  };
}
...
async function importAndPredictOnce() {
  ...
  try {
    ...
    // TÌM phiên đã dự đoán TRƯỚC ĐÓ để so sánh kết quả
    const previousPrediction = data.history.find(h => 
      h.phien === current.phien && 
      h.du_doan && 
      h.du_doan !== "Chưa có"
    );
    
    if (previousPrediction && (!previousPrediction.ket_qua || previousPrediction.ket_qua === "Chưa có")) {
      // Có dự đoán nhưng chưa có kết quả → CẬP NHẬT kết quả thực tế
      ...
      if (!previousPrediction.da_tinh_stats) {
        ...
        if (previousPrediction.du_doan === current.ket_qua) {
          ...
          console.log(`✅ Phiên ${current.phien}: Dự đoán ${previousPrediction.du_doan} = KQ ${current.ket_qua} → ĐÚNG ✓`);
        } else {
          data.stats.sai++;
          data.flow.lastLosses = (data.flow.lastLosses || 0) + 1;
          data.flow.lastWins = 0;
          data.flow.lastPredictionCorrect = false;
          console.log(`❌ Phiên ${current.phien}: Dự đoán ${previousPrediction.du_doan} ≠ KQ ${current.ket_qua} → SAI ✗`);
        }
        
        previousPrediction.da_tinh_stats = true;
        save();
      }
      
      // KHÔNG return ở đây - tiếp tục tạo dự đoán cho phiên tiếp theo
    }
    ...
    // Kiểm tra công thức (cho trường hợp đặc biệt)
    const formulaResult = formulaBasedPredict(current.tong_xuc_xac, current.xuc_xac);
    
    // Nếu công thức khuyên Skip hoặc tổng 11 → KHÔNG dự đoán
    if (formulaResult.du_doan === "Skip" || current.tong_xuc_xac === 11) {
      console.log(`⏸️ Nghỉ phiên ${nextPhien}: ${formulaResult.reason || 'Tổng 11 - random'}`);
      return { ok: true, processed: false, reason: "Skip prediction" };
    }
    
    // THIẾU dữ liệu (< 10 phiên) → dùng CÔNG THỨC
    if (completedHistory.length < 10) {
      nextDecision = {
        du_doan: formulaResult.du_doan,
        reason: `Công thức (${completedHistory.length} phiên): ${formulaResult.reason}`,
        confidence: formulaResult.confidence,
        details: { pattern: { name: "Công thức Sunwin", type: "formula" } }
      };
    } 
    // ĐỦ dữ liệu (>= 10 phiên) → CHỈ dùng THUẬT TOÁN AI
    else {
...
app.get("/sunwinapi", (req, res) => {
  try {
    ...
    // Tạo pattern sequence (lowercase t/x) - TRÁI→PHẢI = CŨ→MỚI
    const completedHistory = data.history.filter(h => h.ket_qua && h.ket_qua !== "Chưa có");
    const last10 = completedHistory.slice(0, 10); // Lấy 10 phiên mới nhất
    const patternSeq = last10.reverse().map(h => (h.ket_qua[0] === 'T' ? 't' : 'x')).join(""); // Đảo ngược: cũ→mới
