// BOTRUMSUNWIN HYBRIDPlus v2 — Full server (fixed stats + pattern)
// Node.js + Express — giữ nguyên logic gốc, thêm fixes requested
// By @minhsangdangcap (2025) — completed for Render
...
// ====== PATTERN CẦU (T/X) ======
function getPatternCau(hist, n = 20) {
  if (!hist || !hist.length) return "";
  const recent = hist.slice(-n).map(h => (h.ket_qua === "Tài" ? "T" : "X"));
  return recent.join(" ");
}
...
async function fetchAndLearn(){
  try {
    ...
    // build entry (prediction for this phien based on current history)
    let entry = null;
    if(data.history.length < MIN_HISTORY_FOR_AI){
      ...
      // compare with previous prediction if exists (to update stats correctly)
      if(data.history.length > 0){
        const prev = data.history[data.history.length - 1];
        if(prev && prev.du_doan){
          data.stats.tong++;
          if(prev.du_doan === ket_qua) data.stats.dung++; else data.stats.sai++;
        }
      } else {
        // no prev prediction — compare current (fallback) prediction with actual
        data.stats.tong++;
        if(entry.du_doan === ket_qua) data.stats.dung++; else data.stats.sai++;
      }
    ...
    // --- FIXED: update stats by comparing the *previous* prediction with current real result ---
    if(data.history.length > 0){
      const prev = data.history[data.history.length - 1];
      if(prev && prev.du_doan){
        data.stats.tong++;
        ...
        // if prev exists but without du_doan, fall back to comparing current entry.du_doan
        data.stats.tong++;
        if(entry.du_doan === ket_qua) data.stats.dung++; else data.stats.sai++;
      }
    } else {
      // no previous history (edge case) -> compare current prediction with actual
      data.stats.tong++;
      if(entry.du_doan === ket_qua) data.stats.dung++; else data.stats.sai++;
    }
...
app.get("/sunwinapi", (req, res) => {
  ...
  res.json({
    ...
    Pattern_cầu: getPatternCau(data.history, 20),         // <-- added pattern here
...
app.post("/update", (req, res) => {
  try {
    ...
    // Update stats: prefer comparing previous prediction with this real result
    if(data.history.length > 0){
      const prev = data.history[data.history.length - 1];
      if(prev && prev.du_doan){
        data.stats.tong++;
        ...
        // fallback compare current prediction
        data.stats.tong++;
        if(entry.du_doan === ket_qua) data.stats.dung++; else data.stats.sai++;
      }
    } else {
